...
- The npm installation banner now suggests Jenkins `job` and `build` commands
  instead of copied OpenObserve `stream` and `search` commands.
...
- **The CLI now flags which context your commands will hit when several are
  ...
  explicitly it silently uses the saved `current_context` and can inspect the
  wrong Jenkins instance. Now, gated on `>1` context (single-context setups see
  ...
  explicitly (`--use-context` or `JENKINS_CONTEXT`); opt out entirely with
  `JENKINS_CLI_NO_CONTEXT_HINT=1`.
...
- **An unknown subcommand of a command group no longer looks like success.** A
  ...
  `auth`, `job`, `build`, `queue`, `skill`) now returns a structured
...
- **The API client is now an importable Go library.** The HTTP client that
  powers the CLI moved out of `internal/` into `pkg/` (`pkg/apiclient`, `pkg/transport`, `pkg/errors` and `pkg/timeutil`), so external
...
[Unreleased]: https://github.com/AngelMsger/jenkins-cli/compare/v0.4.0...HEAD
...
[0.2.1]: https://github.com/AngelMsger/jenkins-cli/compare/v0.2.0...v0.2.1
[0.2.0]: https://github.com/AngelMsger/jenkins-cli/compare/v0.1.4...v0.2.0
