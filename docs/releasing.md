...
`jenkins-cli` is distributed from **GitHub Releases**. Everything else — the
...
- The repository is public at `github.com/AngelMsger/jenkins-cli`.
- The npm account owns the `@angelmsger` scope; `@angelmsger/jenkins-cli`
...
- **npm publishing uses trusted publishing (OIDC)** — no long-lived token, no
  ...
  npmjs.com → the `@angelmsger/jenkins-cli` **package** → Settings → Trusted
  Publisher → **GitHub Actions**:

  | Field | Value |
  |-------|-------|
  | Organization or user | `AngelMsger` |
  | Repository | `jenkins-cli` |
...
3. **`repository.url` casing must match the GitHub repo exactly.** Provenance
   ...
   repo reported by the OIDC provenance (`AngelMsger/jenkins-cli`); a casing
...
5. sets the npm package version to the tag (minus the `v`) and runs
   `npm publish --access public` for `@angelmsger/jenkins-cli`, skipping the
...
```
jenkins-cli-darwin-amd64    jenkins-cli-linux-amd64    jenkins-cli-windows-amd64.exe
jenkins-cli-darwin-arm64    jenkins-cli-linux-arm64    jenkins-cli-windows-arm64.exe
checksums.txt
```

Download URL pattern:
`https://github.com/AngelMsger/jenkins-cli/releases/download/v<version>/<asset>`

## Companion Skill

The `jenkins` Skill is **embedded into the binary** at build time
(`//go:embed all:skills/jenkins`, see `assets.go`), so every release ships a
Skill that matches the CLI version; users deploy it with `jenkins-cli skill
