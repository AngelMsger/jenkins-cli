'use strict';
// install.js downloads the prebuilt jenkins-cli binary that matches the host
...
const REPO = 'angelmsger/jenkins-cli';
...
function assetName() {
  ...
  return `jenkins-cli-${goos}-${goarch}` + (goos === 'windows' ? '.exe' : '');
}

// binPath returns the directory and file path for the installed binary.
function binPath() {
  const dir = path.join(__dirname, 'binary');
  const exe = process.platform === 'win32' ? 'jenkins-cli.exe' : 'jenkins-cli';
...
function httpGet(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    https
      .get(url, { headers: { 'User-Agent': 'jenkins-cli-npm-installer' } }, (res) => {
...
function welcomeText() {
  return [
    '',
    'jenkins-cli is ready. First-time setup:',
    '',
    '  jenkins-cli config init --pretty   configure your server + credentials (interactive)',
    '  jenkins-cli skill install          install the coding-agent Skill',
    '',
    'Everyday use:',
    '  jenkins-cli stream list',
    '  jenkins-cli search run --stream <name> --since 1h --limit 20',
    '  jenkins-cli --help',
    '',
    'Docs: https://github.com/AngelMsger/jenkins-cli',
    '',
  ].join('\n');
}
...
if (require.main === module) {
  install()
    .then((file) => {
      process.stdout.write(`jenkins-cli: installed ${file}\n`);
    })
    .catch((err) => {
      process.stderr.write(
        `jenkins-cli: postinstall download skipped (${err.message}); ` +
