#!/usr/bin/env node
'use strict';
// Thin launcher for the jenkins-cli binary. It execs the platform binary
...
async function main() {
  ...
  if (!fs.existsSync(file)) {
    process.stderr.write('jenkins-cli: downloading binary...\n');
    await install();
  }

  const res = spawnSync(file, process.argv.slice(2), { stdio: 'inherit' });
  if (res.error) {
    process.stderr.write(`jenkins-cli: ${res.error.message}\n`);
    process.exit(1);
  }
...
main().catch((err) => {
  process.stderr.write(`jenkins-cli: ${err.message}\n`);
  process.exit(1);
});
