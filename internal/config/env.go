...
var envBindings = map[string]string{
	"JENKINS_AUTH_SCHEME":    fieldAuthScheme,
	"JENKINS_CREDENTIAL_URL": fieldCredentialURL,
	"JENKINS_URL":            fieldServer,
	"JENKINS_USER":           fieldAuthUsername,
	"JENKINS_FORMAT":         fieldFormat,
	"JENKINS_PASSWORD":       fieldPassword,
	"JENKINS_TOKEN":          fieldToken,
	"JENKINS_CLI_READ_ONLY":  fieldReadOnly,
}
...
func layerFromVars(vars map[string]string, serviceOnly ...bool) map[string]string {
	...
	if v := vars["JENKINS_AUTH_SCHEME"]; v != "" {
		m[fieldAuthScheme] = v
	}
	return m
}
