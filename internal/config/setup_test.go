...
func cleanSetupEnv(t *testing.T) {
	...
	t.Setenv("JENKINS_CONTEXT", "")
}
func setupFixture() NamedContext {
	return NamedContext{Name: "team", BaseURL: "https://service.example.test/deploy", Auth: AuthConfig{Scheme: "token", Username: "PersonalUser", CredentialURL: "https://help.example.test/tokens"}}
}
...
func TestSetupTargetsNamedContextAndExcludesPersonalEnvironment(t *testing.T) {
	...
	t.Setenv("JENKINS_CONTEXT", "not-defined")
	t.Setenv("JENKINS_FORMAT", "ndjson")
	t.Setenv("JENKINS_USER", "InjectedUser")
	t.Setenv("JENKINS_PASSWORD", "must-not-be-copied")
	dotenv := filepath.Join(dir, "presets.env")
	if err := os.WriteFile(dotenv, []byte("JENKINS_URL=https://dotenv.example.test/deploy\nJENKINS_CREDENTIAL_URL=https://help.example.test/new\nJENKINS_PASSWORD=dotenv-secret\nJENKINS_USER=dotenv-user\n"), 0600); err != nil {
		t.Fatal(err)
	}
	...
	t.Setenv("JENKINS_URL", "https://env.example.test")
	t.Setenv("JENKINS_AUTH_SCHEME", "basic")
	got, err = Load(opt)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCredentialGuideIsDisplayOnlyAndKeepsDeploymentPath(t *testing.T) {
	...
	if g.CredentialURL != nc.BaseURL+"/me/security" {
