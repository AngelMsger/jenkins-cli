...
type LoadOptions struct {
	...
	// over JENKINS_CONTEXT and the file's current_context.
	Context string
}
...
// JENKINS_CONTEXT env var, the file's current_context, the sole context,
...
func selectContext(f File, flagCtx, envCtx string) (string, error) {
	...
	case envCtx != "":
		return pick(envCtx, "JENKINS_CONTEXT")
...
func unknownContextHint(name string, available []string) string {
	...
	return "Run `jenkins-cli config contexts` to list defined contexts."
}
...
func Load(opt LoadOptions) (*Resolved, error) {
	...
	ctxName, err := selectContext(file, opt.Context, os.Getenv("JENKINS_CONTEXT"))
	if err != nil {
		return nil, err
	}
