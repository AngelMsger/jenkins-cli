	...
	form := huh.NewForm(
		huh.NewGroup(
			...
			huh.NewSelect[string]().
				Title("Auth scheme").
				Options(
					huh.NewOption("token — username + API token (recommended)", auth.SchemeToken),
					huh.NewOption("basic — username + password", auth.SchemeBasic),
				).
				Value(&v.scheme),
		),
		huh.NewGroup(
			huh.NewInput().
				Title("API token").
				Description("User → Configure → API Token in Jenkins").
...
func runInitPrompts(def initValues) (initValues, error) {
	...
	if v.scheme, err = promptLine("Auth scheme (token/basic)", v.scheme); err != nil {
		return v, err
	}
	label := "API token"
