...
func newConfigInitCmd(s *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Collects a server URL, username and API token (or password), verifies\n" +
			"them, then writes a named context plus the secret. With --pretty it runs\n" +
			"an interactive TUI (requires a terminal); without it, a plain line-by-line\n" +
			...
			"JENKINS_URL / JENKINS_USER / JENKINS_TOKEN (or JENKINS_PASSWORD) instead.",
		Example: "  jenkins-cli config init --pretty   # interactive TUI (recommended)\n" +
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if vals.scheme != auth.SchemeBasic && vals.scheme != auth.SchemeToken {
				return cerrors.Newf(cerrors.CategoryUsage, "BAD_SCHEME",
					"unknown auth scheme %q (want token or basic)", vals.scheme)
			}
			normURL, err := apiclient.NormalizeBaseURL(vals.baseURL)
			if err != nil {
				return err
			}
			cred := auth.Credential{Scheme: vals.scheme, Username: vals.username, Secret: vals.secret}
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !ok {
				return cerrors.New(cerrors.CategoryConfig, "NO_CONFIG",
					"no config file yet").WithNextSteps("jenkins-cli config init")
			}
			...
			if !found {
				return cerrors.Newf(cerrors.CategoryConfig, "UNKNOWN_CONTEXT",
					...
					WithNextSteps("jenkins-cli config contexts")
			}
