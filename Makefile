BINARY      := jenkins-cli
PKG         := github.com/angelmsger/jenkins-cli
...
build:
	CGO_ENABLED=0 go build -trimpath -ldflags "$(LDFLAGS)" -o bin/$(BINARY) ./cmd/jenkins-cli
...
# Prefer `jenkins-cli skill install` (auto-detects agent dirs).
install-skill: build
